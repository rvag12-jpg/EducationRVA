...
(() => {
  ...
  function removeEnrollment(db,enrollmentId){
    ...
    (db.snapshots||[]).forEach(s=>{ if(Array.isArray(s.results)) s.results=s.results.filter(r=>r.enrollment_id!==eid); });
  ...
  function removeGroup(db,groupId){
    ...
    studentIds.forEach(sid=>{ if(!db.enrollments.some(e=>e.student_id===sid)) db.students=db.students.filter(s=>s.id!==sid); });
  ...
  window.localApi = async function(url,opts={}){
    ...
    let m;
    if(method==='DELETE' && (m=path.match(/^\/api\/enrollments\/(\d+)$/))){ const db=exportDb(),out=removeEnrollment(db,Number(m[1]));saveDb(db);return ok(out); }
    if(method==='DELETE' && (m=path.match(/^\/api\/groups\/(\d+)$/))){ const db=exportDb(),out=removeGroup(db,Number(m[1]));saveDb(db);if(activeGroupId===out.groupId)activeGroupId=null;return ok(out); }
  ...
  function installUi(){ const heading=document.querySelector('.group-heading');if(heading&&!document.querySelector('#deleteGroupFeature')){const right=heading.lastElementChild,b=document.createElement('button');b.id='deleteGroupFeature';b.className='btn danger';b.textContent='Eliminar grupo';b.onclick=deleteGroup;if(right&&right.classList.contains('row'))right.appendChild(b);else heading.appendChild(b);}document.querySelectorAll('[data-observe]').forEach(observe=>{const row=observe.closest('.list-row');if(!row||row.querySelector('[data-delete-enrollment]'))return;const b=document.createElement('button');b.className='btn danger';b.textContent='Eliminar';b.dataset.deleteEnrollment=observe.dataset.observe;b.onclick=()=>deleteStudent(observe);const action=observe.parentElement;if(action&&action.classList.contains('row'))action.appendChild(b);else{const wrap=document.createElement('div');wrap.className='row';observe.replaceWith(wrap);wrap.appendChild(observe);wrap.appendChild(b);}}); }
  const observer=new MutationObserver(()=>installUi());observer.observe(document.getElementById('app')||document.body,{childList:true,subtree:true});installUi();
})();
